...
export async function POST(request: Request) {
  try {
    ...
    // Locally: LALAL.AI is the primary path only for the env-gated demo
    // track; any failure/timeout falls back to Demucs. All other local
    // tracks use Demucs directly. The LALAL API key is only ever used
    // server-side here — never sent to the client.
    const arrayBuffer = await file.arrayBuffer();
    const buffer = Buffer.from(arrayBuffer);

    let result = null as Awaited<ReturnType<typeof separateVocals>> | null;
    if (shouldUseLalal(trackId)) {
      console.log(`[/api/upload] Trying LALAL.AI for track ${trackId}`);
